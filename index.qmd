...
# Multi-modal input {background-image="images/section-slide.png"}
...
```{.python code-line-numbers="|1|4|5|"}
chat = chatlas.ChatAnthropic(model="claude-sonnet-4-6", api_key=os.getenv("ANTHROPIC_API_KEY"))
...
Context & Challenge                                                                                
 ...
 • The Monterrey Metropolitan Area (MMA) is one of Mexico's top three GHG-emitting territories     
   (30.34 MtCO₂e), with population doubling and urban area growing 2.8x between 1990-2020          
 • Local governments lacked technical capabilities and sufficient data to support effective climate
   decision-making                                                                                 

Key Approach                                                                                       

 • Combined local climate data with Google's Environmental Insights Explorer (EIE) platform,       
   alongside complementary tools (Maitec for mobility, Tecsb for green/solar technologies)         
 • Engaged diverse stakeholders — government, private sector, academia, and marginalized           
   communities — through workshops and participatory sessions                                      

Main Outcomes                                                                                      

 • Generated more robust, data-driven information to support Climate Action Plans (CAPs)           
 • Identified opportunities in solar energy potential, sustainable buildings, and low-emission     
   transportation                                                                                  
 • Increased transparency and accountability in climate action planning                            
 • Contributed to better-informed public policies at both municipal and state levels               

Critical Lesson                                                                                    

 • Stakeholder engagement was the most decisive factor for success — particularly identifying      
   influential community members and including marginalized voices to ensure inclusive,            
...
# Packages that work with chatlas

## Chat with your data in R and Python with querychat

:::: {.columns}

::: {.column width="10%"}

![](images/querychat-logo.png)

:::

::: {.column width="90%"}

![](images/querychat-example.png){fig-align="center" width="80%"}
...
## Natural Language Processing (NLP) with mall
...
![](images/mall-example.png){style="box-shadow: 5px 5px 15px rgba(0, 0, 0, 0.3); border-radius: 5px;" width=80% }
